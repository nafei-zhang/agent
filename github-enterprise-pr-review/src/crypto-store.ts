...
export async function encryptSecret(secret: string): Promise<CipherPayload> {
...
export async function decryptSecret(payload: CipherPayload): Promise<string> {
  try {
    const key = await readOrCreateKey();
    ...
    throw new AppError("DECRYPT_FAILED", "解密敏感信息失败，请重新初始化配置。", error);
  }
}
